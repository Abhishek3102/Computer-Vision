...
class FaceMeshDetector():
    def __init__(self, staticMode=False, maxFaces=2, minDetectionCon=0.5, minTrackCon=0.5):
        ...
        self.faceMesh = self.mpFaceMesh.FaceMesh(
            static_image_mode=self.staticMode,  # Boolean
            max_num_faces=self.maxFaces,  # Integer
            min_detection_confidence=self.minDetectionCon,  # Float
            min_tracking_confidence=self.minTrackCon  # Float
        )
        self.drawSpec = self.mpDraw.DrawingSpec(thickness=1, circle_radius=2)
...
def main():
    ...
    while True:
        ...
        if key & 0xFF == ord('q'):
            ...
            img_name = f"face_mesh_module{time.time()}.png"
